...
function dereference_range(range) {
    ...
    return {
        sheet: m[1],
        s: decode_cell(m[2]+m[3]),
        e: m[4] !== undefined ? decode_cell(m[4]+m[5]) : undefined
    }
}
  ...
  var side = null;
	(t[0].match(tagregex)||[]).forEach(function(x) {
		...
		switch(strip_ns(y[0])) {
			...
			case '<left': 
      case '<left>': 
        side = border.left = {style: y.style};
        break;
			case '</left>':
        side = null;
        break;
			...
			case '<right': 
      case '<right>': 
        side = border.right = {style: y.style};
        break;
			case '</right>': 
        side = null;
        break;
      ...
      case '<top>': 
        ...
        break;
			...
			case '<bottom': 
      case '<bottom>':
        side = border.bottom = {style: y.style};
        break;
			case '</bottom>': 
        side = null;
        break;

			/* 18.8.6 diagonal CT_BorderPr */
			case '<diagonal/>':
			case '<diagonal': 
      case '<diagonal>':
        side = border.diagonal = {style: y.style};
        break;
			case '</diagonal>': 
        side = null;
        break;

			/* 18.8.6 horizontal CT_BorderPr */
			case '<horizontal/>':
			case '<horizontal': 
      case '<horizontal>':
        side = border.horizontal = {style: y.style};
        break;
			case '</horizontal>': 
        side = null;
        break;

			/* 18.8.6 vertical CT_BorderPr */
			case '<vertical/>':
			case '<vertical': 
      case '<vertical>':
        side = border.vertical = {style: y.style};
        break;
			case '</vertical>': 
        side = null;
        break;
...
function parse_zip(zip, opts) {
  ...
  Object.values(out['Sheets']).forEach(sheet => {
      Object.entries(sheet).forEach(([k,v]) => {
          if(k.match(/^!/) || v.s===undefined) {
              return;
          }
          const styleXf = styles['CellXf'][v.s];
          const style = v.style = {};
          if(styleXf.applyBorder) {
              style.border = styles['Borders'][styleXf.borderId];
          }
          if(styleXf.fontId !== undefined) {
              style.font = styles['Fonts'][styleXf.fontId];
          }
          if(styleXf.applyAlignment || styleXf.applyalignment) {
              style.alignment = styleXf.alignment;
          }
          style.fill = styles['Fills'][styleXf.fillId];
      });
  });
...
var utils = {
  ...
  dereference_range: dereference_range,
